      ...
      (fn [fileset]
        ...
        (let [cljs-edns (relevant-cljs-edns (b/fileset-diff @prev-pre fileset) ids)
          ...
          (if-not (empty? cljs-edns)
            ...
            (do (when (empty? @prev-pre)
                  (butil/warn "WARNING: No .cljs.edn file found on the fileset, figreload might misbehave.\n"))
                ;; Special case: boot-cljs used without .cljs.edn
                ;; in that case we can just create client file for the "main"
                ;; id with any name and it will be required by boot-cljs.
